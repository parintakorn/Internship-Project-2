...
const MonthlyBarChart = ({ data, height = 180 }) => {
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <svg width={totalW} height={height + 50} style={{ fontFamily: 'sans-serif', display: 'block' }}>
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              {rH > 18 && <text x={x + barW / 2} y={height - rH + 12} textAnchor="middle" fontSize={8} fill="#fff" fontWeight="bold">{Math.round(+d.received_g / 1000)}k</text>}
              {wH > 18 && <text x={x + barW + gap + barW / 2} y={height - wH + 12} textAnchor="middle" fontSize={8} fill="#fff" fontWeight="bold">{Math.round(+d.withdrawn_g / 1000)}k</text>}
              <text x={x + barW + gap / 2} y={height + 16} textAnchor="middle" fontSize={10} fill="#888">{d.label}</text>
            </g>
          );
        })}
        <rect x={20} y={height + 30} width={10} height={10} fill="#3b82f6" rx={2} />
        <text x={34} y={height + 39} fontSize={11} fill="#666">รับเข้า (กรัม)</text>
        <rect x={130} y={height + 30} width={10} height={10} fill="#ef4444" rx={2} />
        <text x={144} y={height + 39} fontSize={11} fill="#666">เบิกออก (กรัม)</text>
      </svg>
    </div>
  );
};
...
const buildItemMap = (received, withdrawn) => {
  ...
  (received || []).forEach(r => {
    ...
    itemMap[key].received_g  += Number(r.original_weight ?? r.received_weight ?? r.weight ?? 0);
  ...
  return (
    <div style={{ paddingLeft: 4 }}>
      {items.map((item, i) => {
        ...
        const wt   = isIn ? (item.original_weight ?? item.received_weight ?? item.weight ?? 0) : (item.amount ?? 0);
        const dt   = item.created_at || item.receive_date || item.withdraw_date || '';
        return (
          <div key={i} style={{ display: 'flex', gap: 10, position: 'relative', marginBottom: 8 }}>
            {i < items.length - 1 && (
...
// ✅ FIX 1: display:'none' (เหมือน monthly) — ลบ pageBreakInside ทั้งหมด
const PrintDocument = ({ data, year, totalReceivedG, totalWithdrawnG, allMovements }) => {
  if (!data) return null;
    ...
    <div className="print-document" style={{ display: 'none' }}>
      ...
      <div style={{ background: '#F5E6D3', border: '2px solid #C8A882', borderRadius: 6, padding: '14px 18px', marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 800, color: '#1e293b' }}>📈 รายงานคลังวัตถุดิบรายปี</div>
            <div style={{ fontSize: 11, color: '#666', marginTop: 2 }}>Warehouse Yearly Report</div>
          </div>
          <div style={{ textAlign: 'right', fontSize: 11, color: '#444', lineHeight: 1.9 }}>
            <div><strong>ปี:</strong> พ.ศ. {+year + 543} ({year})</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 14 }}>
        {[
          { label: 'รับเข้ารวมทั้งปี',  value: fmtNum(totalReceivedG / 1000) + ' kg',  color: '#3b82f6', bg: '#eff6ff' },
          { label: 'เบิกออกรวมทั้งปี',  value: fmtNum(totalWithdrawnG / 1000) + ' kg', color: '#ef4444', bg: '#fef2f2' },
          { label: 'สุทธิทั้งปี',       value: (totalReceivedG - totalWithdrawnG >= 0 ? '+' : '') + fmtNum((totalReceivedG - totalWithdrawnG) / 1000) + ' kg', color: totalReceivedG - totalWithdrawnG >= 0 ? '#10b981' : '#ef4444', bg: totalReceivedG - totalWithdrawnG >= 0 ? '#f0fdf4' : '#fef2f2' },
          { label: 'ความเคลื่อนไหวรวม', value: fmtNum(allMovements.length, 0) + ' รายการ', color: '#8b5cf6', bg: '#f5f3ff' },
      ...
      {data.by_category?.filter(d => +d.received_g > 0 || +d.withdrawn_g > 0).length > 0 && (
        <div style={{ marginBottom: 14 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
            ...
            <tbody>
              {data.by_category.filter(d => +d.received_g > 0 || +d.withdrawn_g > 0).map((d, i) => { const net = +d.received_g - +d.withdrawn_g; return (
                <tr key={i} style={{ background: i % 2 === 0 ? 'white' : '#f9fafb' }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: '#1e293b', marginBottom: 6, paddingBottom: 5, borderBottom: '2px solid #C8A882' }}>📅 สรุปรายเดือน</div>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
          <thead>
            <tr>{['เดือน','รับเข้า (ก.)','เบิกออก (ก.)','สุทธิ (ก.)','% เบิก'].map(h => (
              <th key={h} style={{ ...thP, textAlign: h === 'เดือน' ? 'left' : 'right' }}>{h}</th>
            ))}</tr>
          </thead>
          <tbody>
            {data.monthly.map((m, i) => {
              const net = +m.received_g - +m.withdrawn_g;
              const pct = +m.received_g > 0 ? ((+m.withdrawn_g / +m.received_g) * 100).toFixed(1) : '0.0';
              return (
                <tr key={i} style={{ background: i % 2 === 0 ? 'white' : '#f9fafb' }}>
                  <td style={{ ...tdP, textAlign: 'left', fontWeight: 700 }}>{m.label}</td>
                  <td style={{ ...tdP, textAlign: 'right', color: '#3b82f6', fontWeight: +m.received_g > 0 ? 700 : 400 }}>{fmtNum(m.received_g)}</td>
                  <td style={{ ...tdP, textAlign: 'right', color: '#ef4444', fontWeight: +m.withdrawn_g > 0 ? 700 : 400 }}>{fmtNum(m.withdrawn_g)}</td>
                  <td style={{ ...tdP, textAlign: 'right', fontWeight: 800, color: net >= 0 ? '#10b981' : '#ef4444' }}>{net >= 0 ? '+' : ''}{fmtNum(net)}</td>
                  <td style={{ ...tdP, textAlign: 'right', fontWeight: 700 }}>{pct}%</td>
                </tr>
              );
            })}
          </tbody>
          <tfoot>
            <tr style={{ background: '#e8dcc8' }}>
              <td style={{ ...tdP, textAlign: 'left', fontWeight: 800 }}>รวมทั้งปี</td>
              <td style={{ ...tdP, textAlign: 'right', fontWeight: 800, color: '#3b82f6' }}>{fmtNum(totalReceivedG)}</td>
              <td style={{ ...tdP, textAlign: 'right', fontWeight: 800, color: '#ef4444' }}>{fmtNum(totalWithdrawnG)}</td>
              <td style={{ ...tdP, textAlign: 'right', fontWeight: 800, color: totalReceivedG - totalWithdrawnG >= 0 ? '#10b981' : '#ef4444' }}>{fmtNum(totalReceivedG - totalWithdrawnG)}</td>
              <td style={tdP} />
            </tr>
          </tfoot>
        </table>
      ...
      {data.top_withdrawn?.length > 0 && (
        <div style={{ marginBottom: 14 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
            ...
            <tbody>
              {data.top_withdrawn.map((d, i) => { const pct = totalReceivedG > 0 ? ((+d.withdrawn_g / totalReceivedG) * 100).toFixed(1) : '0.0'; return (
                <tr key={i} style={{ background: i % 2 === 0 ? 'white' : '#f9fafb' }}>
...
const WarehouseReportYearly = () => {
  ...
  const fetchReport = async (y) => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/admin/warehouse/report/yearly?year=${y}`, { headers: authHeaders() });
  ...
  const totalReceivedG  = data?.monthly.reduce((s, m) => s + +m.received_g, 0) || 0;
  const totalWithdrawnG = data?.monthly.reduce((s, m) => s + +m.withdrawn_g, 0) || 0;
  const peakMonth = data?.monthly.reduce((best, m) => +m.received_g > +best.received_g ? m : best, { received_g: 0, label: '-' });
  ...
  const exportExcel = () => {
    ...
    if (allMovements.length) {
      const movRows = allMovements.map(m => ({ 'ประเภท': m.type === 'receive' ? 'รับเข้า' : 'เบิกออก', 'รหัส': m.code, 'ชื่อ': m.name, 'หมวดหมู่': m.category, 'น้ำหนัก (ก.)': m.type === 'receive' ? (m.original_weight ?? m.received_weight ?? m.weight) : m.amount, 'เหตุผล': m.reason || '-', 'วันที่': m.receive_date || m.withdraw_date }));
    ...
    const monthRows = data.monthly.map(m => ({ 'เดือน': m.label, 'รับเข้า (ก.)': m.received_g, 'เบิกออก (ก.)': m.withdrawn_g, 'สุทธิ': +m.received_g - +m.withdrawn_g }));
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(monthRows), 'รายเดือน');
    const topRows = data.top_withdrawn.map((d, i) => ({ 'อันดับ': i + 1, 'ชื่อ': d.name, 'หมวดหมู่': d.category, 'เบิกรวม (ก.)': d.withdrawn_g }));
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(topRows), 'Top 10');
    XLSX.writeFile(wb, `warehouse_yearly_${year}.xlsx`);
  };

  // ✅ FIX 2: ใช้ setTimeout 800ms แทน onload เพื่อให้ browser render เสร็จก่อน print
  //           ไม่ auto-close หน้าต่าง ปล่อยให้ user ปิดเอง
  const handlePrint = () => {
    ...
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><style>
      ...
      body { margin: 0; padding: 12px; font-family: sans-serif; background: white; }
  ...
  return (
    <div style={{ padding: '1.5rem', background: '#f5f5f5', minHeight: '100vh' }}>
      ...
      <PrintDocument data={data} year={year} totalReceivedG={totalReceivedG} totalWithdrawnG={totalWithdrawnG} allMovements={allMovements} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <div>
          <h1 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#1a1a2e', margin: 0 }}>📈 รายงานรายปี</h1>
          <p style={{ color: '#999', margin: '0.2rem 0 0', fontSize: '0.85rem' }}>ภาพรวมคลังวัตถุดิบตลอดทั้งปี พร้อมรายละเอียดความเคลื่อนไหว</p>
        </div>
      </div>
      ...
      <div className="rc" style={{ background: 'white', borderRadius: 12, padding: '1rem 1.25rem', marginBottom: '1.25rem', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: '#aaa', textTransform: 'uppercase', letterSpacing: '0.06em' }}>📅 เลือกปี</span>
        <select value={year} onChange={e => setYear(e.target.value)} style={{ padding: '8px 12px', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, outline: 'none', background: 'white' }}>
          {yearOptions.map(y => <option key={y} value={y}>{+y + 543} ({y})</option>)}
        </select>
        <button onClick={() => fetchReport(year)} style={{ padding: '8px 18px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: 8, fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>🔍 ดูข้อมูล</button>
        {data && <>
      ...
      {!loading && data && (
        <div>
          <div className="rc" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(170px,1fr))', gap: '1rem', marginBottom: '1.25rem' }}>
            <StatCard color="#3b82f6" icon="📥" label="รับเข้ารวม"          value={fmtNum(totalReceivedG / 1000)}  sub="กิโลกรัม" />
            <StatCard color="#ef4444" icon="📤" label="เบิกออกรวม"          value={fmtNum(totalWithdrawnG / 1000)} sub="กิโลกรัม" />
            <StatCard color="#10b981" icon="📊" label="สุทธิทั้งปี"         value={(totalReceivedG - totalWithdrawnG >= 0 ? '+' : '') + fmtNum((totalReceivedG - totalWithdrawnG) / 1000)} sub="กิโลกรัม" />
            <StatCard color="#f59e0b" icon="📌" label="เดือนที่รับมากสุด"   value={peakMonth?.label || '-'} sub={peakMonth?.received_g ? `${fmtNum(+peakMonth.received_g / 1000, 1)} kg` : ''} />
            {allMovements.length > 0 && <StatCard color="#8b5cf6" icon="🔄" label="ความเคลื่อนไหวรวม" value={fmtNum(allMovements.length, 0)} sub="รายการ" />}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: '1rem', marginBottom: '1.25rem' }}>
            <div className="rc" style={{ background: 'white', borderRadius: 12, padding: '1.25rem', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              <h3 style={{ margin: '0 0 1rem', fontSize: '1rem', fontWeight: 700, color: '#1a1a2e' }}>📊 รับเข้า vs เบิกออก ทั้งปี</h3>
          ...
          <div className="rc" style={{ background: 'white', borderRadius: 12, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
            <div style={{ display: 'flex', borderBottom: '2px solid #e5e7eb', overflowX: 'auto' }}>
              {allMovements.length > 0 && <button style={tabStyle('movement')} onClick={() => setActiveTab('movement')}>🔄 ความเคลื่อนไหว ({allMovements.length})</button>}
              <button style={tabStyle('monthly')}  onClick={() => setActiveTab('monthly')}>📅 รายเดือน</button>
            ...
            {activeTab === 'movement' && allMovements.length > 0 && (
            ...
            <div style={{ padding: activeTab !== 'movement' ? '1rem' : 0, overflowX: 'auto' }}>
              {activeTab === 'monthly' && (
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead><tr style={{ background: '#1e293b' }}>{['เดือน','รับเข้า (ก.)','เบิกออก (ก.)','สุทธิ (ก.)','% เบิก'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr></thead>
                  <tbody>
                    {data.monthly.map((m, i) => { const net = +m.received_g - +m.withdrawn_g; const pct = +m.received_g > 0 ? ((+m.withdrawn_g / +m.received_g) * 100).toFixed(1) : '0.0'; return (
                      <tr key={i} style={{ background: i % 2 === 0 ? 'white' : '#f9fafb' }}>
                        <td style={{ ...tdStyle, fontWeight: 700 }}>{m.label}</td>
                        <td style={{ ...tdStyle, color: '#3b82f6', fontWeight: +m.received_g > 0 ? 700 : 400 }}>{fmtNum(m.received_g)}</td>
                        <td style={{ ...tdStyle, color: '#ef4444', fontWeight: +m.withdrawn_g > 0 ? 700 : 400 }}>{fmtNum(m.withdrawn_g)}</td>
                        <td style={{ ...tdStyle, color: net >= 0 ? '#10b981' : '#ef4444', fontWeight: 700 }}>{net >= 0 ? '+' : ''}{fmtNum(net)}</td>
                        <td style={tdStyle}><span style={{ background: +pct > 80 ? '#fee2e2' : +pct > 50 ? '#fef3c7' : '#d1fae5', color: +pct > 80 ? '#dc2626' : +pct > 50 ? '#d97706' : '#065f46', padding: '2px 8px', borderRadius: 20, fontSize: 12, fontWeight: 700 }}>{pct}%</span></td>
                      </tr>
                    );})}
                    <tr style={{ background: '#f0f9ff' }}>
                      <td style={{ ...tdStyle, fontWeight: 800 }}>รวมทั้งปี</td>
                      <td style={{ ...tdStyle, fontWeight: 800, color: '#3b82f6' }}>{fmtNum(totalReceivedG)}</td>
                      <td style={{ ...tdStyle, fontWeight: 800, color: '#ef4444' }}>{fmtNum(totalWithdrawnG)}</td>
                      <td style={{ ...tdStyle, fontWeight: 800, color: totalReceivedG - totalWithdrawnG >= 0 ? '#10b981' : '#ef4444' }}>{fmtNum(totalReceivedG - totalWithdrawnG)}</td>
                      <td style={tdStyle} />
                    </tr>
                  </tbody>
                </table>
              ...
              {activeTab === 'top' && (
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {data.top_withdrawn.length === 0
                      ? <tr><td colSpan={6} style={{ padding: 24, textAlign: 'center', color: '#ccc', fontStyle: 'italic' }}>ไม่มีการเบิกในปีนี้</td></tr>
                      : data.top_withdrawn.map((d, i) => { const pct = totalReceivedG > 0 ? ((+d.withdrawn_g / totalReceivedG) * 100).toFixed(1) : '0.0'; return (
                        <tr key={i} style={{ background: i % 2 === 0 ? 'white' : '#f9fafb' }}>
                          ...
                          <td style={tdStyle}><span style={{ background: +pct > 10 ? '#fee2e2' : '#f3f4f6', color: +pct > 10 ? '#dc2626' : '#666', borderRadius: 20, padding: '2px 8px', fontSize: 12, fontWeight: 700 }}>{pct}%</span></td>
                        </tr>
                      );})}
                  </tbody>
                </table>
            ...
            <div style={{ flex: 1, background: isIn ? '#fafcff' : '#fffafa', borderRadius: 10, padding: '10px 14px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: `1px solid ${isIn ? '#bfdbfe' : '#fecaca'}` }}>
              ...
              <div style={{ display: 'flex', gap: 16, marginTop: 7, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 15, fontWeight: 800, color: isIn ? '#3b82f6' : '#ef4444' }}>
                  {isIn ? '+' : '-'}{fmtNum(isIn ? (item.original_weight ?? item.received_weight ?? item.weight) : item.amount)} ก.
                  <span style={{ fontSize: 11, color: '#aaa', fontWeight: 400, marginLeft: 4 }}>({fmtNum((isIn ? (item.original_weight ?? item.received_weight ?? item.weight) : item.amount) / 1000, 3)} kg)</span>
                </span>
...
export default WarehouseReportYearly;